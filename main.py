...
lang_keyboard = ReplyKeyboardMarkup(
    [[lang] for lang in LANGUAGES.keys()],  # tagma-tag joylash
    resize_keyboard=True
)
...
async def translate_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Statistika (faqat Behruz uchun)
    if str(user_id) == '6905227976' and user_text == '/stat':
        count = sum(len(h) for h in user_history.values())
        await update.message.reply_text(f"Bot umumiy {count} ta tarjima bajargan.", reply_markup=main_keyboard)
        return
